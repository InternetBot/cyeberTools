...
# Function to get Radeon GPU temperature
def get_gpu_temperature():
    try:
        # Iterate over all sensors, looking specifically for GPU temperature
        for sensor in wmi_client.Sensor():
            if sensor.SensorType == 'Temperature' and 'GPU' in sensor.Name:
                return f"{sensor.Value}°C"
    except Exception as e:
        logging.error(f"Error fetching GPU temperature: {e}")
        return "Error"
    return "N/A"
